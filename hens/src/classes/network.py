...
class Network:
    ...
    def __init__(self,
                 ...
                 utility_sigmas: dict[tuple[Utility, TemperatureInterval], float],
                 utility_deltas: dict[tuple[Utility, TemperatureInterval], float]) -> None:
        ...
        self.T: list[TemperatureInterval] = min_utility_instance.intervals  # K_q
        self.P = min_utility_instance.accepted_h_c  # P_ij
        self.Pk = min_utility_instance.accepted_h_c_k  # P_ijk
        self.diff_t_min: float = min_utility_instance.diff_t_min
        self.sigmas: dict[tuple[Union[Stream, Utility], TemperatureInterval], float] = min_utility_instance.sigmas  # Qh_ik
        self.deltas: dict[tuple[Union[Stream, Utility], TemperatureInterval], float] = min_utility_instance.deltas  # Qc_jk
        self.heats: dict[Stream, float] = {}   # hi
        self.demands: dict[Stream, float] = {}  # cj
        self.U: dict[(Any, Any), float] = {}  # Qu_ij
        self.U_greedy: dict[(Any, Any), float] = {}  # Qu_ij
        self.u_ijk: dict[tuple[Stream, Stream, TemperatureInterval], float] = {}  # Qu_ijk
        self.u_ijkl: dict[tuple[Stream, TemperatureInterval, Stream, TemperatureInterval], float] = {}  # Qu_ik,jl
    ...
    def __update_sigmas(self, new_sigmas: dict[tuple[Utility, TemperatureInterval], float]) -> None:
        for key in new_sigmas.keys():
            self.sigmas[key] = new_sigmas[key]
    
    def __update_deltas(self, new_deltas: dict[tuple[Utility, TemperatureInterval], float]) -> None:
